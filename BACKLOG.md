...
**概要**
Claude Code のセッションが複数同時に走っている場合、現状は「最後に `update_status` を呼んだセッションが上書き」するため、1つしかモニタリングできない。

**対応案**
- MCP 起動時に UUID でセッションIDを生成
- `state.js` を `Map<sessionId, state>` 構造に変更
- `/api/status` で全セッション分を返す
- UI を複数パネルに分割表示（2セッション以上で自動拡張）

**優先度**: 中（複数 CC 窓で作業する場合に有用）
